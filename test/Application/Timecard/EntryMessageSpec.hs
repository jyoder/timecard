...
spec = do
    describe "createAll" $ do
        beforeAll (testConfig >>= mockContext RootApplication) do
            it "inserts a timecard entry message for each Twilio message" $ withContext do
    ...
    describe "replaceAll" $ do
        beforeAll (testConfig >>= mockContext RootApplication) do
            it "replaces existing timecard entry messages with new ones" $ withContext do
                withTransactionRollback do
                    ron <-
                        newRecord @Person
                            |> set #firstName "Ronald"
    ...
    describe "fetchByTimecardEntry" $ do
        beforeAll (testConfig >>= mockContext RootApplication) do
            it "returns timecard entry messages associated with the given timecard entry" $ withContext do
                withTransactionRollback do
                    ron <-
                        newRecord @Person
                            |> set #firstName "Ronald"
