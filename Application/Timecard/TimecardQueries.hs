...
buildTimecard (firstRow : otherRows) =
    Just
        Timecard
            ...
            , status = buildStatus firstRow
            , entries = buildTimecardEntry <$> firstRow : otherRows
            }

buildStatus :: ResultRow -> Status
buildStatus resultRow =
    let (id, value, expiresAt, isRevoked) =
            ( get #accessTokenId resultRow
            , get #accessTokenValue resultRow
            , get #accessTokenExpiresAt resultRow
            , get #accessTokenIsRevoked resultRow
            )
     in case (id, value, expiresAt, isRevoked) of
            (Just id, Just value, Just expiresAt, Just isRevoked) ->
                Reviewing $ AccessToken {..}
            _ -> Created
